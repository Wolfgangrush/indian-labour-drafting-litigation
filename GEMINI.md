# GEMINI.md — Indian Labour Law Drafting (Gemini CLI port)
...
**Plugin:** `indian-labour-drafting` · **Forum:** State Industrial Court / Labour Court / Conciliation Officer / Industrial Tribunal / Authority under the applicable state Shops & Establishments Act / Authority under Code on Wages 2019 / OSH Code 2020 · **Publisher:** Wolfgang Rush · **License:** MIT

---

## 🇮🇳 WHO YOU ARE TALKING TO

The user is an **enrolled Indian advocate** drafting labour, industrial-disputes, employment-rights, social-security, and workplace-protection pleadings for filing. They are running the 6-agent drafting pipeline (reader → format → drafter → verifier → refiner → overseer) and expect filing-grade output (.docx) with anti-hallucination discipline.
...
Every pipeline run reads the user's `state-config.md + case-config.md` from the case folder. Substitute every `{{config.X}}` placeholder in the format-shell with the real value before passing to the Drafter.
...
- Preserve the advocate's bench's idiom (substituted from state-config.md + case-config.md)

### Rule 8 — STOP on missing inputs
- Reader STOPS if required law PDF missing
- Format STOPS if state-config.md + case-config.md missing or incomplete
...
```
<case-folder>/
├── state-config.md + case-config.md    ← required, advocate-supplied
...
Industrial dispute under ID Act 1947 (or IR Code 2020 once notified) · Shops & Establishments dispute · ESI claim · PF dispute · Gratuity claim · Bonus claim · Maternity Benefit · Code on Wages 2019 claim · OSH Code 2020 claim · Code on Social Security 2020 claim · Sexual Harassment (POSH Act 2013) Internal Committee complaint

Each case type has its own template in `skills/ (labour case-type templates + state exemplars)`.
